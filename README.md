...
```
python create_def.py [--init] [--reset-input] [--force] [--no-verbose-input] [--log-level LEVEL] <work_dir>
```

| Flag | Default | Description |
|------|---------|-------------|
| `--init` | off | Run initialisation step (extract data, generate `.inp`) |
| `--reset-input` | off | With `--init`: delete existing `.inp` and temp cache files before regenerating |
| `--force` | off | Build step only: push the build through past `.inp` validation errors and missing required fields instead of blocking, writing best-effort output (raw/blank values where a field couldn't be resolved). Disables safety checks — errors are still printed in full, but nothing stops the build. Only use this if you understand the data and are prepared to fix up the result by hand |
...
- A failed download is retried a couple of times (transient connection failures happen occasionally against exomol.com), then logged and skipped rather than stopping the rest; the script exits non-zero if anything was never obtained.
...
- `validator.py` — file-discovery and file-structure checks. `discover_dataset` scans `work_dir` for `*.states` files and parses `{iso_slug}__{ds_name}` from filenames (isotopologue slugs must start with a digit, e.g. `27Al-1H`). `validate_iso_files` enforces exactly one `.states`, exactly one `.pf`, and ≥1 `.trans` file per isotopologue, with a content spot-check. Also owns `_data_files` (filename-based file lookup) and `_open_maybe_bz2` (a context manager yielding a text-mode file handle, streaming `.bz2` decompression via `bz2.open` rather than materializing the full decompressed content on disk — needed since some `.trans.bz2` files decompress to several GB), which `extractor.py` reuses rather than duplicating.
...
- `inp_handler.py` — the largest module: generates the blank `.inp` template (aligned key-value sections, states-file column preview, standard-label auto-fill from `lib/standard_label_structure.json`), parses the filled `.inp` back into a dict, and validates its contents (`validate_inp`, returning `(errors, warnings)` — warnings require a `y/N` confirmation before build, errors block, `--force` overrides both). `parse_inp` keeps a non-blank value as raw text when it fails its normal type coercion (rather than dropping the field), so `--force` has something to write through. User-facing message strings for parsing/validation live in module-level dicts (`_VALIDATION_ERRORS`, `_VALIDATION_WARNINGS`) near the top of the file — edit messages there without touching logic. The `.inp` is read with `encoding="utf-8-sig"` so a BOM added by Windows editors doesn't break section-header parsing.
- `merger.py` — `deep_merge`/`merge_iso` combine the auto-derived temp cache with user-supplied `.inp` fields (a `None` user value means "keep auto-derived"); `validate_complete` checks a fixed list of required dict paths (`_REQUIRED_PATHS`) before rendering is allowed, unless `--force` is set.
