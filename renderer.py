"""
Template-based .def file rendering.

BaseRenderer defines the interface. ExoMolRenderer translates the exomol.json-shaped
merged dict into the flat ExoMol .def text format.
...
class BaseRenderer(ABC):
    ...
    @abstractmethod
    def name(cls) -> str:
        """Short identifier used in registry and CLI (e.g. 'exomol')."""

    @abstractmethod
    def render(self, merged_dict: dict, output_path: Path) -> None:
        """Write the rendered .def file to output_path."""
...
def _fmt_line(value: str, comment: str) -> str:
    ...
    value = _to_ascii(str(value).strip())
    comment = _to_ascii(str(comment).strip())
...
class ExoMolRenderer(BaseRenderer):
    """
    Renders a fully-merged exomol.json-shaped dict to the ExoMol .def plain-text format.

    Field ordering follows def_structure.txt. Broadening section is omitted.
    """
    ...
    def render(self, d: dict, output_path: Path) -> None:
        logging.info(f"renderer: ExoMolRenderer writing '{output_path}'")
        lines = self._build_lines(d)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        with open(output_path, "w", encoding="utf-8") as f:
            f.writelines(lines)
        logging.info(f"renderer: wrote {len(lines)} lines to '{output_path}'")

    def _build_lines(self, d: dict) -> list[str]:
        out = []
...
REGISTRY: dict[str, type[BaseRenderer]] = {
    ExoMolRenderer.name(): ExoMolRenderer,
}
...
def get_renderer(name: str) -> BaseRenderer:
    ...
    if cls is None:
        raise ValueError(
            f"Unknown renderer '{name}'. Available: {sorted(REGISTRY)}"
        )
    return cls()
